...
export const BottomNavigation = () => {
  ...
  const leftMenu: MenuItem[] = [
    { title: 'Todo', to: PATH.TODO, icon: 'icon-[ph--check-circle]', activeIcon: 'icon-[ph--check-circle-fill]' },
    { title: 'Category', to: PATH.CATEGORY, icon: 'icon-[ph--shapes]', activeIcon: 'icon-[ph--shapes-fill]' }
  ];

  const rightMenu: MenuItem[] = [
    { title: 'Calendar', to: PATH.CALENDAR, icon: 'icon-[ph--calendar]', activeIcon: 'icon-[ph--calendar-fill]' },
  ...
  const renderMenuItem = React.useCallback(
    (item: MenuItem) => {
      ...
      return (
        ...
        >
          <span className={`${selected ? item.activeIcon : item.icon} w-6 h-6`}></span>
